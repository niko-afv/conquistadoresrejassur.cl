...
class Trayectoria_Unidad extends CI_Model{
    ...
    public function save($unidad){
        ...
        if($this->nuevo){
            ...
            if($res && $this->countIntegrantes() > 0){
                $array = array();
                $array['UNIDADES_TRAYECTORIA_ID'] = $this->getId();
                $array['INTEGRANTES_RUT'] = $this->getIntegrante($this->countIntegrantes()-1)->getRut();
                $res = $this->db->insert('UNIDADES_TRAYECTORIA_INTEGRANTES',$array);
            }
        }
        return $res;
    }
    
    public function deleteIntegrante($rut){
        $this->db->where('INTEGRANTES_RUT',$rut);
        $this->db->where('UNIDADES_TRAYECTORIA_ID',  $this->getId());
        $res = $this->db->delete('UNIDADES_TRAYECTORIA_INTEGRANTES');
        return $res;
   }
